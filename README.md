...
2. **进入章节**：选择北京《中轴入局》。
3. **查看棋盘**：棋子落在当前格，右侧显示当前回合角色和任务触发。
4. **打开场景**：进入该格的 16:9 场景任务页。
5. **上传/手选证据**：拍摄现场照片，或用手选元素作为兜底继续。
6. **掷时空骰**：骰面决定剧场语气，如时辰、风物、来信、市声、转折、回声。
7. **角色剧场**：玩家选择追问，角色用打字机对白回应；AI Mode 会实时请求模型重写回应。
8. **完成现实任务**：拍照观察、市声记录或留下回声。
9. **领取卡牌**：获得文化卡、角色卡、线索卡或成就卡。
10. **继续走格**：回到棋盘进入下一地点。
11. **生成游记**：终局页根据卡牌和回声句子生成北京路线故事；AI Mode 会把完整行为时间线交给模型生成个性化游记。
